...
- Он начинается в методе handle(), с получения списка всех пользователей из таблицы "users" и перебирает каждого пользователя, проверяя, если его подписка скоро истекает. Если это так и пользователь подтвердил свою почту, то скрипт отправляет уведомление на электронную почту пользователя.
